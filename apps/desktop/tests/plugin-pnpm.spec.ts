...
it('installs a real pnpm graph, then executes approved scripts with the shared host instance', async () => {
  ...
  try {
    ...
    // Absent by default: this spec covers the real pnpm graph, not bundled plugins.
    const localPlugins = join(root, 'resources', 'local-plugins')
    const manager = new DesktopProjectManager(resolveDesktopPaths(join(root, '.dsh')), { node: process.execPath, pnpm, dsh, localPlugins })
