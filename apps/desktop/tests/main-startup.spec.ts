...
const harness = await vi.hoisted(async () => {
  ...
  return {
    windows, hosts, trays, managerRuntimes, handlers, app, FakeWindow, FakeHost, FakeTray, FakeNativeImage,
    ...
    reset() {
      windows.length = 0; hosts.length = 0; trays.length = 0; managerRuntimes.length = 0; handlers.clear(); app.removeAllListeners()
      app.isPackaged = true
