...
import { DEFAULT_FONT_SIZE, DEFAULT_SKIN, type ThemePreference, type ThemeSkin } from '../theme-settings.ts'

/** Store state mirrored from the theme snapshot. */
export interface AppearanceRowState {
  /** Persisted preference (selection state reads this, never the resolved active theme). */
  preference: ThemePreference
  /** Persisted conversation skin (fork addition). */
  skin: ThemeSkin
...
type AppearanceRowActions = {
  sync: (draft: AppearanceRowState, preference: ThemePreference, skin: ThemeSkin, revision: number) => void
}
...
export function createAppearanceRowStore(): EngineStoreHandle<AppearanceRowState, AppearanceRowActions> {
  return defineStore({
    init: (): AppearanceRowState => ({ preference: 'system', skin: DEFAULT_SKIN, revision: -1 }),
    actions: {
      sync: (d, preference: ThemePreference, skin: ThemeSkin, revision: number) => {
